...
from utils.utils import int2bin, write_to_file, read_file, read_ak_from_file
...
def run_whole_signal(_signal, ws, thu, p, pf, wa, mu, gain_bits, g_max, to_plot=False):
    ...
    ak = np.zeros((int(len(_signal) / ws), 10)).astype(np.float64)
    # ak = ak.tolist()
...
def quantize(pitch, gains, gain_bits, g_max, ak):
...
def quantize_ak(ak):
    ...
    quantized_ak = np.array(quantized_ak)
    print(auxs[2])
    return quantized_ak, auxs
